...
BUILD_TARGET="${BUILD_TARGET:-pcd2web}"
...
if [[ -z "${TOOLCHAIN_FILE}" && -f "${ROOT_DIR}/vcpkg/scripts/buildsystems/vcpkg.cmake" ]]; then
...
"${CMAKE_BIN}" --build "${BUILD_DIR}" --target "${BUILD_TARGET}" -j
